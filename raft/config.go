...
// make the functions to ensure that the leader is unchanged throughout the window
func (cfg *config) checkUnchangedLeader() int {
				...
				if t, leader := cfg.rafts[i].GetState(); leader {
					...
					if leader {
						if initialLeader == -1 {
							...
							if i == initialLeader && initialTerm == t {
								continue
							} else {
								cfg.t.Fatalf("Re-election occured unnecessarily")
								return -1
							}
						}
					}
	...
	if initialLeader == -1 {
		cfg.t.Fatalf("No leader selected, timeout")
		return -1
	} else {
		//cfg.t.Fatalf("Leader remained same")
		return 1
	}
}
...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		if cfg.applyErr[i] != "" {
			fmt.Printf("applyErr[%d] = %v\n", i, cfg.applyErr[i])
			cfg.t.Fatal(cfg.applyErr[i])
		}

		cfg.mu.Lock()
		cmd1, ok := cfg.logs[i][index]
		// fmt.Printf("cmd1: %v, ok: %t\n", cmd1, ok)
