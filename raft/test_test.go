...
func TestSameLeaderElection(t *testing.T) {
	fmt.Printf("Test: leader & term remains unchanged ...\n")
	servers := 3
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()
	result := cfg.checkUnchangedLeader()
	if result == -1 {
...
func TestReElection(t *testing.T) {
	...
	// Checking if a previous leader can again become a leader
	cfg.disconnect(leader3)
	cfg.checkOneLeader()

	// Re adding all leaders back
	cfg.connect(leader3)
	cfg.checkOneLeader()
